//
//  ChimetaHandgame+CosineSimilary.swift
//  ChimetaHandgame
//
...
public extension CHHandInfo {
    ...
    func similarity(of joints: Set<HandSkeleton.JointName>, to vector: CHHandInfo) -> Float {
    ...
    func similarity(of finger: CHJointOfFinger, to vector: CHHandInfo, flexibleJointOnly: Bool = true) -> Float {
        return similarity(of: [finger], to: vector, flexibleJointOnly: flexibleJointOnly)
    }
    /// Fingers your selected
    func similarity(of fingers: Set<CHJointOfFinger>, to vector: CHHandInfo, flexibleJointOnly: Bool = true) -> Float {
        var similarity: Float = 0
    ...
    func similarity(to vector: CHHandInfo, flexibleJointOnly: Bool = true) -> Float {
        return similarity(of: .all, to: vector, flexibleJointOnly: flexibleJointOnly)
    }
    /// all
    func similarities(to vector: CHHandInfo, flexibleJointOnly: Bool = true) -> (average: Float, eachFinger: [CHJointOfFinger: Float]) {
        return averageAndEachSimilarities(of: .all, to: vector, flexibleJointOnly: flexibleJointOnly)
    }
    func averageAndEachSimilarities(of fingers: Set<CHJointOfFinger>, to vector: CHHandInfo, flexibleJointOnly: Bool = true) -> (average: Float, eachFinger: [CHJointOfFinger: Float]) {
        let fingerTotal = fingers.reduce(into: [CHJointOfFinger: Float]()) { partialResult, finger in
        ...
        let fingerScore = fingerTotal.reduce(into: [CHJointOfFinger: Float]()) { partialResult, ele in
